...
This high-priority snapshot describes only the current scene. Generated output permits only the canonical `# Story State` heading and the four ordered H2 sections above. Every section needs meaningful CommonMark content: ordinary text, non-empty code, or non-empty rendered image-alt text. Empty list markers, thematic breaks, and images without rendered alt text are not meaningful. Generated output must not contain raw HTML outside code. It must not maintain plot-state markers or decide whether a plot thread is complete; `summary.md` and immutable episodes retain the full story arc.

### 5.3 `summary.md`

用于保存面向 Prompt 的整体剧情回顾，覆盖早期事件、人物关系、关键变化、承诺和伏笔事实。新生成内容采用无 Markdown 标题的连续叙事，可使用普通段落、列表或代码，但不得包含代码之外的原始 HTML，也不使用剧情线状态标记或判断剧情线、伏笔是否完成。旧摘要中的标题、旧状态标记和原始 HTML 可以作为迁移输入；成功更新后的新摘要必须符合无标题、无原始 HTML 格式。
...
正常更新只创建新章节，不重写旧章节。Episode H1 和消息范围元数据由应用写入；模型生成的正文只包含上述三个二级标题，每节都遵循与 Story State 相同的有效内容规则，并拒绝 CommonMark 识别为代码之外原始 HTML 的任何 token，包括 HTML 注释。代码围栏、缩进代码或行内代码中的 HTML 与 `messages:` 字面量仍是正文。`undo` 或 `retry` 若触及已覆盖区间，则删除受影响区间及其后的派生章节并重新生成。除 Episode H1 和上述三个二级标题外，不允许其他真实 Markdown 标题；代码围栏内的标题字面量不计入结构。

读取已有 episode 时只要求不可歧义的 canonical envelope 和非空正文：文件名与 H1 使用六位 ASCII 数字，文件中恰好有一个代码之外的 canonical ASCII 消息范围注释，编号和范围形成连续链。旧正文可以包含额外历史标题、`[open]`、`[closed]`、`[done]` 标记和原始 HTML，供复用和 summary 迁移使用，读取时绝不改写。完整现有链不得超过本次提供的最高 ChatRecord 编号；已有 1-20 且精确提供 21-40 这类切片输入仍然合法。Empty records define history end 0, but the complete existing chain is still validated before returning; any stale episode raises without rewrite so summary cannot resurrect removed history.
...
- 已有 `memory.md`、`summary.md`、`story_state.md`、episode、角色档案和资产直接沿用；历史标题、状态标记或原始 HTML 仅作为迁移输入，不触发生成输出校验。
