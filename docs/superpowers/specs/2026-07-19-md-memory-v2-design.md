...
它是高优先级的当前场景快照，只描述“现在正在发生什么”。除 `# Story State` 和上述四个二级标题外，不允许其他真实 Markdown 标题；代码围栏内的标题字面量不计入结构。它不维护 `[open]`、`[closed]` 等剧情线状态，也不判断剧情线是否完成；完整剧情脉络统一由 `summary.md` 和不可变 episode 保留。
