...
它是高优先级的当前场景快照，只描述“现在正在发生什么”。除 `# Story State` 和上述四个二级标题外，不允许其他真实 Markdown 标题；代码围栏内的标题字面量不计入结构。四节都必须包含 CommonMark token 可识别的有效内容，不能只含 HTML 注释、空列表/分隔线或无替代文本的图片；普通文本、非空代码内容、图片替代文本和可见 HTML 文本均有效。它不维护 `[open]`、`[closed]` 等剧情线状态，也不判断剧情线是否完成；完整剧情脉络统一由 `summary.md` 和不可变 episode 保留。

### 5.3 `summary.md`

用于保存面向 Prompt 的整体剧情回顾，覆盖早期事件、人物关系、关键变化、承诺和伏笔事实。新生成内容采用无 Markdown 标题的连续叙事，可使用普通段落或列表，但不使用剧情线状态标记，也不判断剧情线或伏笔是否完成。旧摘要中的标题和旧状态标记可以作为迁移输入；成功更新后的新摘要必须符合无标题格式。
...
正常更新只创建新章节，不重写旧章节。Episode H1 和消息范围元数据由应用写入；模型生成的正文只包含上述三个二级标题，每节都遵循与 Story State 相同的有效内容规则，并拒绝 CommonMark 识别为真实 HTML 的任何 `messages:` 注释。代码围栏、缩进代码或行内代码中的同名字面量仍是正文。`undo` 或 `retry` 若触及已覆盖区间，则删除受影响区间及其后的派生章节并重新生成。除 Episode H1 和上述三个二级标题外，不允许其他真实 Markdown 标题；代码围栏内的标题字面量不计入结构。

读取已有 episode 时只要求不可歧义的 canonical envelope 和非空正文：文件名与 H1 使用六位 ASCII 数字，文件中恰好有一个代码之外的 canonical ASCII 消息范围注释，编号和范围形成连续链。旧正文可以包含额外历史标题和 `[open]`、`[closed]`、`[done]` 标记，供复用和 summary 迁移使用，读取时绝不改写。完整现有链不得超过本次提供的最高 ChatRecord 编号；已有 1-20 且精确提供 21-40 这类切片输入仍然合法。
