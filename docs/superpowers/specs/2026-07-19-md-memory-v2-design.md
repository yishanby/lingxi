...
读取已有 episode 时只要求不可歧义的 canonical envelope 和非空正文：文件名与 H1 使用六位 ASCII 数字，文件中恰好有一个代码之外的 canonical ASCII 消息范围注释，编号和范围形成连续链。旧正文可以包含额外历史标题、`[open]`、`[closed]`、`[done]` 标记和原始 HTML，供复用和 summary 迁移使用，读取时绝不改写。完整现有链不得超过本次提供的最高 ChatRecord 编号；已有 1-20 且精确提供 21-40 这类切片输入仍然合法。
