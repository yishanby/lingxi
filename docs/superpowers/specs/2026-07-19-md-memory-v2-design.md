...
data/memory/{session_id}/
  ...
  story_state.md          # 当前时间、地点、在场角色、状态和未完成剧情线
...
它是高优先级上下文，更新时必须保留尚未解决的剧情线，只有剧情明确完成时才能移除或标记为 closed。
...
章节是不可变的历史压缩单元，每个文件包含消息范围、参与角色、关键事件、状态变化、承诺和未完成伏笔。例如：
...
3. **当前故事层**：完整 `story_state.md`，特别是未完成剧情线。
